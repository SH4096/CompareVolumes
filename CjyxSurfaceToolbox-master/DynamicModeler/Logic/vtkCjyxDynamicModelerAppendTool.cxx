...
//----------------------------------------------------------------------------
vtkToolNewMacro(vtkCjyxDynamicModelerAppendTool);
...
//----------------------------------------------------------------------------
vtkCjyxDynamicModelerAppendTool::vtkCjyxDynamicModelerAppendTool()
{
  /////////
  // Inputs
...
//----------------------------------------------------------------------------
const char* vtkCjyxDynamicModelerAppendTool::GetName()
{
  return "Append";
}

//----------------------------------------------------------------------------
bool vtkCjyxDynamicModelerAppendTool::RunInternal(vtkDRMLDynamicModelerNode* surfaceEditorNode)
{
  ...
  vtkDRMLModelNode* outputModelNode = vtkDRMLModelNode::SafeDownCast(surfaceEditorNode->GetNodeReference(APPEND_OUTPUT_MODEL_REFERENCE_ROLE));
  if (!outputModelNode)
    {
    // Nothing to output
    return true;
  ...
  for (int i = 0; i < numberOfInputNodes; ++i)
    {
    vtkDRMLNode* inputNode = surfaceEditorNode->GetNthNodeReference(APPEND_INPUT_MODEL_REFERENCE_ROLE, i);
    vtkDRMLModelNode* modelNode = vtkDRMLModelNode::SafeDownCast(inputNode);
    if (!modelNode)
      {
      continue;
      }
...
//----------------------------------------------------------------------------
bool vtkCjyxDynamicModelerAppendTool::RemoveDuplicateCells(vtkPolyData* input)
{
