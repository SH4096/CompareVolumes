...
#include <vtkDRMLMarkupsPlaneNode.h>
...
vtkCjyxDynamicModelerBoundaryCutTool::vtkCjyxDynamicModelerBoundaryCutTool()
{
  ...
  inputMarkupEvents->InsertNextTuple1(vtkDRMLMarkupsNode::PointModifiedEvent);
  inputMarkupEvents->InsertNextTuple1(vtkDRMLTransformableNode::TransformModifiedEvent);

  vtkNew<vtkStringArray> inputBorderClassNames;
  inputBorderClassNames->InsertNextValue("vtkDRMLMarkupsCurveNode");
  inputBorderClassNames->InsertNextValue("vtkDRMLMarkupsPlaneNode");
  ...
  inputSeedFiducialClassNames->InsertNextValue("vtkDRMLMarkupsFiducialNode");
  NodeInfo inputSeed(
    "Seed point node",
...
//----------------------------------------------------------------------------
bool vtkCjyxDynamicModelerBoundaryCutTool::RunInternal(vtkDRMLDynamicModelerNode* surfaceEditorNode)
{
  ...
  vtkDRMLModelNode* outputModelNode = vtkDRMLModelNode::SafeDownCast(surfaceEditorNode->GetNodeReference(OUTPUT_MODEL_REFERENCE_ROLE));
  if (!outputModelNode)
    {
    // Nothing to output
    return true;
  ...
  vtkDRMLModelNode* inputModelNode = vtkDRMLModelNode::SafeDownCast(surfaceEditorNode->GetNodeReference(INPUT_MODEL_REFERENCE_ROLE));
  if (!inputModelNode)
    {
    // Nothing to output
    return true;
  ...
  for (int i = 0; i < numberOfInputNodes; ++i)
    {
    vtkDRMLNode* inputNode = surfaceEditorNode->GetNthNodeReference(INPUT_BORDER_REFERENCE_ROLE, i);

    vtkNew<vtkPolyData> outputLinePolyData;
    vtkDRMLMarkupsPlaneNode* planeNode = vtkDRMLMarkupsPlaneNode::SafeDownCast(inputNode);
    if (planeNode)
      {
  ...
  DRMLNodeModifyBlocker blocker(outputModelNode);
  outputModelNode->SetAndObserveMesh(outputPolyData);
  outputModelNode->InvokeCustomModifiedEvent(vtkDRMLModelNode::MeshModifiedEvent);
...
//----------------------------------------------------------------------------
void vtkCjyxDynamicModelerBoundaryCutTool::GetSeedPoints(vtkDRMLDynamicModelerNode* surfaceEditorNode, vtkPoints* seedPoints)
{
  ...
  vtkDRMLMarkupsFiducialNode* seedNode = vtkDRMLMarkupsFiducialNode::SafeDownCast(
...
//----------------------------------------------------------------------------
void vtkCjyxDynamicModelerBoundaryCutTool::GetDefaultSeedPoint(vtkDRMLDynamicModelerNode * surfaceEditorNode, double defaultSeedPoint[3])
{
  if (!surfaceEditorNode)
    {
