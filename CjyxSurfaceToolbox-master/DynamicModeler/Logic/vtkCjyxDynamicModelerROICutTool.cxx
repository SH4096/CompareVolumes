...
// DRML includes
#include <vtkDRMLMarkupsROINode.h>
...
vtkCjyxDynamicModelerROICutTool::vtkCjyxDynamicModelerROICutTool()
{
  ...
  inputModelEvents->InsertNextTuple1(vtkDRMLModelNode::MeshModifiedEvent);
  inputModelEvents->InsertNextTuple1(vtkDRMLTransformableNode::TransformModifiedEvent);
  ...
  inputROIEvents->InsertNextTuple1(vtkDRMLMarkupsNode::PointModifiedEvent);
  inputROIEvents->InsertNextTuple1(vtkDRMLTransformableNode::TransformModifiedEvent);
  vtkNew<vtkStringArray> inputROIClassNames;
  inputROIClassNames->InsertNextValue("vtkDRMLMarkupsROINode");
  NodeInfo inputROI(
    "ROI node",
...
//----------------------------------------------------------------------------
bool vtkCjyxDynamicModelerROICutTool::RunInternal(vtkDRMLDynamicModelerNode* dynamicModelerNode)
{
  ...
  vtkDRMLModelNode* outputInsideModelNode = vtkDRMLModelNode::SafeDownCast(dynamicModelerNode->GetNodeReference(this->GetOutputInsideModelReferenceRole()));
  vtkDRMLModelNode* outputOutsideModelNode = vtkDRMLModelNode::SafeDownCast(dynamicModelerNode->GetNodeReference(this->GetOutputOutsideModelReferenceRole()));
  ...
  vtkDRMLModelNode* inputModelNode = vtkDRMLModelNode::SafeDownCast(dynamicModelerNode->GetNodeReference(this->GetInputModelReferenceRole()));
  if (!inputModelNode)
    {
  ...
  vtkDRMLMarkupsROINode* roiNode = vtkDRMLMarkupsROINode::SafeDownCast(dynamicModelerNode->GetNodeReference(this->GetInputROIReferenceRole()));
  if (!roiNode)
    {
  ...
  vtkDRMLTransformNode::GetTransformBetweenNodes(inputModelNode->GetParentTransformNode(), nullptr, this->InputModelNodeToWorldTransform);
  ...
  if (roiType != vtkDRMLMarkupsROINode::ROITypeBox && roiType != vtkDRMLMarkupsROINode::ROITypeBoundingBox)
    {
    capSurface = false;
    }
  ...
  if (outputInsideModelNode)
    {
    vtkDRMLTransformNode::GetTransformBetweenNodes(nullptr, outputInsideModelNode->GetParentTransformNode(), this->OutputInsideWorldToModelTransform);
    this->OutputInsideWorldToModelTransformFilter->Update();

    DRMLNodeModifyBlocker blocker(outputInsideModelNode);
    ...
    outputInsideModelNode->InvokeCustomModifiedEvent(vtkDRMLModelNode::MeshModifiedEvent);
    }

  if (outputOutsideModelNode)
    {
    vtkDRMLTransformNode::GetTransformBetweenNodes(nullptr, outputOutsideModelNode->GetParentTransformNode(), this->OutputOutsideWorldToModelTransform);
    this->OutputOutsideWorldToModelTransformFilter->Update();

    DRMLNodeModifyBlocker blocker(outputOutsideModelNode);
    ...
    outputOutsideModelNode->InvokeCustomModifiedEvent(vtkDRMLModelNode::MeshModifiedEvent);
    }
