...
/// DRML includes
#include <vtkDRMLDisplayableNode.h>
#include <vtkDRMLDisplayNode.h>
...
//---------------------------------------------------------------------------
vtkDRMLNode* vtkCjyxDynamicModelerTool::GetNthInputNode(int n, vtkDRMLDynamicModelerNode* surfaceEditorNode)
{
...
//---------------------------------------------------------------------------
vtkDRMLNode* vtkCjyxDynamicModelerTool::GetNthOutputNode(int n, vtkDRMLDynamicModelerNode* surfaceEditorNode)
{
  if(!surfaceEditorNode)
    {
    vtkErrorMacro("Invalid parameter node");
...
//---------------------------------------------------------------------------
vtkVariant vtkCjyxDynamicModelerTool::GetNthInputParameterValue(int n, vtkDRMLDynamicModelerNode* surfaceEditorNode)
{
  if (n >= this->GetNumberOfInputParameters())
    {
    vtkErrorMacro("Parameter " << n << " is out of range!");
...
//---------------------------------------------------------------------------
bool vtkCjyxDynamicModelerTool::HasRequiredInputs(vtkDRMLDynamicModelerNode* surfaceEditorNode)
{
...
//---------------------------------------------------------------------------
bool vtkCjyxDynamicModelerTool::HasOutput(vtkDRMLDynamicModelerNode* surfaceEditorNode)
{
...
//---------------------------------------------------------------------------
void vtkCjyxDynamicModelerTool::GetInputNodes(vtkDRMLDynamicModelerNode* surfaceEditorNode, std::vector<vtkDRMLNode*>& inputNodes)
{
  for (int inputIndex = 0; inputIndex < this->GetNumberOfInputNodes(); ++inputIndex)
    ...
    for (int referenceIndex = 0; referenceIndex < numberOfNodeReferences; ++referenceIndex)
      {
      vtkDRMLNode* inputNode = surfaceEditorNode->GetNthNodeReference(referenceRole.c_str(), referenceIndex);
      if (inputNode)
        {
...
//---------------------------------------------------------------------------
void vtkCjyxDynamicModelerTool::GetOutputNodes(vtkDRMLDynamicModelerNode* surfaceEditorNode, std::vector<vtkDRMLNode*>& outputNodes)
{
  for (int outputIndex = 0; outputIndex < this->GetNumberOfOutputNodes(); ++outputIndex)
    ...
    for (int referenceIndex = 0; referenceIndex < numberOfNodeReferences; ++referenceIndex)
      {
      vtkDRMLNode* outputNode = surfaceEditorNode->GetNthNodeReference(referenceRole.c_str(), referenceIndex);
      if (outputNode)
        {
...
//---------------------------------------------------------------------------
void vtkCjyxDynamicModelerTool::CreateOutputDisplayNodes(vtkDRMLDynamicModelerNode* surfaceEditorNode)
{
  if (!surfaceEditorNode)
    {
  ...
  std::vector<vtkDRMLNode*> outputNodes;
  this->GetOutputNodes(surfaceEditorNode, outputNodes);

  for (vtkDRMLNode* outputNode : outputNodes)
    {
    vtkDRMLDisplayableNode* outputDisplayableNode = vtkDRMLDisplayableNode::SafeDownCast(outputNode);
    ...
    vtkDRMLDisplayNode* outputDisplayNode = outputDisplayableNode->GetDisplayNode();
    if (outputDisplayNode == nullptr)
      {
      continue;
      }

    for (vtkDRMLNode* inputNode : inputNodes)
      {
      vtkDRMLDisplayableNode* inputDisplayableNode = vtkDRMLDisplayableNode::SafeDownCast(inputNode);
      if (inputDisplayableNode == nullptr || !inputDisplayableNode->IsA(outputNode->GetClassName()))
        {
        continue;
        }

      vtkDRMLDisplayNode* inputDisplayNode = inputDisplayableNode->GetDisplayNode();
      if (!inputDisplayNode)
        {
        continue;
        }
...
//---------------------------------------------------------------------------
bool vtkCjyxDynamicModelerTool::Run(vtkDRMLDynamicModelerNode* surfaceEditorNode)
{
