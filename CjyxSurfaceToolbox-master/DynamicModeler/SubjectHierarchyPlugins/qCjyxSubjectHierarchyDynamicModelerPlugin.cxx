...
// SubjectHierarchy DRML includes
#include "vtkDRMLSubjectHierarchyNode.h"
#include "vtkDRMLSubjectHierarchyConstants.h"
...
double qCjyxSubjectHierarchyDynamicModelerPlugin::canAddNodeToSubjectHierarchy(
  vtkDRMLNode* node, vtkIdType parentItemID/*=vtkDRMLSubjectHierarchyNode::INVALID_ITEM_ID*/)const
{
  ...
  else if (node->IsA("vtkDRMLDynamicModelerNode"))
    {
    return 0.5;
    }
  return 0.0;
...
{
  if (itemID == vtkDRMLSubjectHierarchyNode::INVALID_ITEM_ID)
    {
    qCritical() << Q_FUNC_INFO << ": Invalid input item";
    return 0.0;
    }
  vtkDRMLSubjectHierarchyNode* shNode = qCjyxSubjectHierarchyPluginHandler::instance()->subjectHierarchyNode();
  if (!shNode)
    {
  ...
  vtkDRMLNode* associatedNode = shNode->GetItemDataNode(itemID);
  if (associatedNode && associatedNode->IsA("vtkDRMLDynamicModelerNode"))
    {
    return 0.5;
    }
...
QIcon qCjyxSubjectHierarchyDynamicModelerPlugin::icon(vtkIdType itemID)
{
  ...
  vtkDRMLDynamicModelerNode* associatedNode = vtkDRMLDynamicModelerNode::SafeDownCast(shNode->GetItemDataNode(itemID));
...
void qCjyxSubjectHierarchyDynamicModelerPlugin::continuousUpdateChanged()
{
  ...
  vtkDRMLScene* scene = qCjyxSubjectHierarchyPluginHandler::instance()->drmlScene();
  if (!scene)
    {
    qCritical() << Q_FUNC_INFO << ": Invalid DRML scene";
    return;
    }
  vtkIdType currentItemID = qCjyxSubjectHierarchyPluginHandler::instance()->currentItem();
  vtkDRMLNode* node = shNode->GetItemDataNode(currentItemID);
  vtkDRMLDynamicModelerNode* dynamicModelerNode = vtkDRMLDynamicModelerNode::SafeDownCast(node);
