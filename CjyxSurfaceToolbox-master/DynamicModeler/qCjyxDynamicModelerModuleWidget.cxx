...
#include <qDRMLSubjectHierarchyModel.h>
...
public:
  ...
  vtkWeakPointer<vtkDRMLDynamicModelerNode> DynamicModelerNode{ nullptr };
...
void qCjyxDynamicModelerModuleWidget::onAddToolClicked()
{
  ...
  if (!QObject::sender() || !this->drmlScene())
    {
    return;
    }

  QString toolName = QObject::sender()->property("ToolName").toString();
  std::string nodeName = this->drmlScene()->GenerateUniqueName(toolName.toStdString());

  vtkNew<vtkDRMLDynamicModelerNode> dynamicModelerNode;
  dynamicModelerNode->SetName(nodeName.c_str());
  dynamicModelerNode->SetToolName(toolName.toUtf8());
  this->drmlScene()->AddNode(dynamicModelerNode);
...
{
  Q_D(qCjyxDynamicModelerModuleWidget);
  if (!this->drmlScene() || this->drmlScene()->IsBatchProcessing())
    {
    return;
    }

  vtkDRMLDynamicModelerNode* meshModifyNode = vtkDRMLDynamicModelerNode::SafeDownCast(d->SubjectHierarchyTreeView->currentNode());
  qvtkReconnect(d->DynamicModelerNode, meshModifyNode, vtkCommand::ModifiedEvent, this, SLOT(updateWidgetFromDRML()));

  d->DynamicModelerNode = meshModifyNode;
  this->updateWidgetFromDRML();
}
...
bool qCjyxDynamicModelerModuleWidget::isInputWidgetsRebuildRequired()
{
  ...
  QList<qDRMLNodeComboBox*> inputNodeSelectors = d->InputNodesCollapsibleButton->findChildren<qDRMLNodeComboBox*>();
  for (qDRMLNodeComboBox* inputNodeSelector : inputNodeSelectors)
    {
...
void qCjyxDynamicModelerModuleWidget::rebuildInputWidgets()
{
  ...
  for (int inputIndex = 0; inputIndex < tool->GetNumberOfInputNodes(); ++inputIndex)
    ...
    for (int inputSelectorIndex = 0; inputSelectorIndex < numberOfInputs; ++inputSelectorIndex)
      ...
      connect(nodeSelector, SIGNAL(currentNodeChanged(vtkDRMLNode*)),
        this, SLOT(updateDRMLFromWidget()));
      }
    }
}
...
void qCjyxDynamicModelerModuleWidget::updateInputWidgets()
{
  ...
  for (qDRMLNodeComboBox* inputNodeSelector : inputNodeSelectors)
    ...
    vtkDRMLNode* referenceNode = d->DynamicModelerNode->GetNthNodeReference(referenceRole.toUtf8(), inputSelectorIndex);
...
void qCjyxDynamicModelerModuleWidget::updateOutputWidgets()
{
  ...
  QList< qDRMLNodeComboBox*> outputNodeSelectors = d->OutputNodesCollapsibleButton->findChildren<qDRMLNodeComboBox*>();
  for (qDRMLNodeComboBox* outputNodeSelector : outputNodeSelectors)
    {
    QString referenceRole = outputNodeSelector->property("ReferenceRole").toString();
    vtkDRMLNode* referenceNode = d->DynamicModelerNode->GetNodeReference(referenceRole.toUtf8());
...
//-----------------------------------------------------------------------------
void qCjyxDynamicModelerModuleWidget::updateWidgetFromDRML()
{
  Q_D(qCjyxDynamicModelerModuleWidget);
  vtkCjyxDynamicModelerLogic* meshModifyLogic = vtkCjyxDynamicModelerLogic::SafeDownCast(this->logic());
  vtkCjyxDynamicModelerTool* tool = nullptr;
...
//-----------------------------------------------------------------------------
void qCjyxDynamicModelerModuleWidget::updateDRMLFromWidget()
{
  Q_D(qCjyxDynamicModelerModuleWidget);
  if (!d->DynamicModelerNode)
    {
  ...
  DRMLNodeModifyBlocker blocker(d->DynamicModelerNode);
