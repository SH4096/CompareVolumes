...
class SurfaceToolboxWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
  ...
  def setup(self):
    ...
    uiWidget = cjyx.util.loadUI(self.resourcePath('UI/SurfaceToolbox.ui'))
    self.layout.addWidget(uiWidget)
    self.ui = cjyx.util.childWidgetVariables(uiWidget)

    # Set scene in DRML widgets. Make sure that in Qt designer the top-level qDRMLWidget's
    # "drmlSceneChanged(vtkDRMLScene*)" signal in is connected to each DRML widget's.
    # "setDRMLScene(vtkDRMLScene*)" slot.
    uiWidget.setDRMLScene(cjyx.drmlScene)
    ...
    self.addObserver(cjyx.drmlScene, cjyx.drmlScene.StartCloseEvent, self.onSceneStartClose)
    self.addObserver(cjyx.drmlScene, cjyx.drmlScene.EndCloseEvent, self.onSceneEndClose)

    # These connections ensure that whenever user changes some settings on the GUI, that is saved in the DRML scene
  ...
  def initializeParameterNode(self):
    ...
    if not self._parameterNode.GetNodeReference("inputModel"):
      firstModelNode = cjyx.drmlScene.GetFirstNodeByClass("vtkDRMLModelNode")
      if firstModelNode:
  ...
  def onApplyButton(self):
    ...
    try:
      ...
      #cjyx.updateGUIFromParameterNode()
      self.ui.applyButton.text = "Apply"
    except Exception as e:
      cjyx.util.errorDisplay("Failed to compute output model: "+str(e))
      import traceback
      traceback.print_exc()
...
class SurfaceToolboxLogic(ScriptedLoadableModuleLogic):
  ...
  def decimate(inputModel, outputModel, reductionFactor=0.8, decimateBoundary=True, lossless=False, aggressiveness=7.0):
    ...
    cliNode = cjyx.cli.runSync(cjyx.modules.decimation, None, parameters)
    cjyx.drmlScene.RemoveNode(cliNode)
...
class SurfaceToolboxTest(ScriptedLoadableModuleTest):
  ...
  def test_AllProcessing(self):
    ...
    modelNode = SampleData.downloadFromURL(
      ...
      uris='https://github.com/Cjyx/CjyxTestingData/releases/download/SHA256/d5aa4901d186902f90e17bf3b5917541cb6cb8cf223bfeea736631df4c047652',
    ...
    parameterNode = cjyx.drmlScene.AddNewNodeByClass("vtkDRMLScriptedModuleNode")
    logic.setDefaultParameters(parameterNode)

    parameterNode.SetNodeReferenceID("inputModel", modelNode.GetID())

    outputModelNode = cjyx.drmlScene.AddNewNodeByClass("vtkDRMLModelNode", "output")
