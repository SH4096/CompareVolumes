...
Please refer to <a href=\"$a/Documentation/$b.$c/Modules/CompareVolumes\"> the documentation</a>.
    ...
    """).substitute({ 'a':parent.cjyxWikiUrl, 'b':cjyx.app.majorVersion, 'c':cjyx.app.minorVersion })
...
class CompareVolumesWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    #
    self.backgroundSelector = cjyx.qDRMLNodeComboBox()
    self.backgroundSelector.nodeTypes = ( ("vtkDRMLVolumeNode"), "" )
    ...
    self.backgroundSelector.setDRMLScene( cjyx.drmlScene )
    ...
    #
    self.labelSelector = cjyx.qDRMLNodeComboBox()
    self.labelSelector.nodeTypes = ( ("vtkDRMLLabelMapVolumeNode"), "" )
    ...
    self.labelSelector.setDRMLScene( cjyx.drmlScene )
  ...
  def onCompareVolumes(self):
    ...
    volumeNodes = [cjyx.drmlScene.GetNodeByID(id) for id in volumeIDs]
    ...
    if self.hotLinkWithCursorCheck.checked:
        for viewName in viewers.keys():
            sliceWidget = cjyx.app.layoutManager().sliceWidget(viewName)
        ...
        crosshairNode = cjyx.drmlScene.GetSingletonNode("default", "vtkDRMLCrosshairNode")
...
class VolumeOrderSelect:
    ...
    def __init__(self):
      ...
      events = [cjyx.drmlScene.NodeAddedEvent,
                cjyx.drmlScene.NodeRemovedEvent,
                cjyx.drmlScene.NewSceneEvent]
      self.observations = []
      for event in events:
        self.observations.append(cjyx.drmlScene.AddObserver(event, self.refresh))
      self.refresh()
    ...
    def cleanup(self):
      for observation in self.observations:
        cjyx.drmlScene.RemoveObserver(observation)
    ...
    def refresh(self, caller=None, event=None):
      ...
      sceneVolumeNodes = list(cjyx.util.getNodes('*VolumeNode*').values())
      ...
      for (volumeID,volumeCheckState) in zip(volumeIDs, volumeCheckStates):
        volumeNode = cjyx.drmlScene.GetNodeByID(volumeID)
...
class CompareVolumesLogic(ScriptedLoadableModuleLogic):
  ...
  def __init__(self):
    ...
    self.sliceViewItemPattern = """
      <item><view class="vtkDRMLSliceNode" singletontag="{viewName}">
    ...
    self.colors = cjyx.util.getNode('GenericColors')
  ...
  def assignLayoutDescription(self,layoutDescription):
    ...
    layoutNode = cjyx.util.getNode('*LayoutNode*')
    if layoutNode.IsLayoutDescription(layoutNode.CjyxLayoutUserView):
      layoutNode.SetLayoutDescription(layoutNode.CjyxLayoutUserView, layoutDescription)
    else:
      layoutNode.AddLayoutDescription(layoutNode.CjyxLayoutUserView, layoutDescription)
    layoutNode.SetViewArrangement(layoutNode.CjyxLayoutUserView)
  ...
  def viewerPerVolume(self,volumeNodes=None,background=None,label=None,viewNames=[],layout=None,orientation='Axial',opacity=0.5):
    ...
    if not volumeNodes:
      volumeNodes = list(cjyx.util.getNodes('*VolumeNode*').values())
    ...
    for index in range(len(actualViewNames)):
      ...
      compositeNode = sliceWidget.drmlSliceCompositeNode()
      if background:
  ...
  def rotateToVolumePlanes(self, referenceVolume):
    ...
    sliceLogics = cjyx.app.layoutManager().drmlSliceLogics()
...
class ViewWatcher:
  ...
  def processEvent(self,observee,event):
    ...
    nodeEvent = (observee.IsA('vtkDRMLSliceNode') or
                observee.IsA('vtkDRMLSliceCompositeNode'))
    if nodeEvent:
    ...
    if observee in self.sliceWidgetsPerStyle:
      ...
      self.sliceNode = self.sliceWidget.drmlSliceNode()
...
class LayerReveal(ViewWatcher):
  ...
  def __init__(self,parent=None,width=400,height=400,showWidget=False,scale=False):
    ...
    self.drmlUtils = cjyx.qDRMLUtils()
  ...
  def onSliceWidgetEvent(self,event):
    ...
    if event == "LeaveEvent" or not self.layerVolumeNodes['F']:
      ...
      self.drmlUtils.qImageToVtkImageData(revealPixmap.toImage(),self.vtkImage)
      self.imageMapper.SetInputData(self.vtkImage)
      x,y = self.xy
  ...
  def revealPixmap(self, xy):
    ...
    bgQImage = qt.QImage()
    fgQImage = qt.QImage()
    cjyx.qDRMLUtils().vtkImageDataToQImage(bgVTKImage, bgQImage)
    cjyx.qDRMLUtils().vtkImageDataToQImage(fgVTKImage, fgQImage)
...
cjyx.util.mainWindow().moduleSelector().selectModule("CompareVolumes"); cjyx.modules.CompareVolumesWidget.onReloadAndTest(scenario="LayerReveal"); reveal = LayerReveal()
    """
